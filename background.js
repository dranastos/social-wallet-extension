...
if (typeof Buffer === 'undefined') {
    console.log('🔧 Installing Buffer polyfill for service worker environment');
    ...
    console.log('✅ Buffer polyfill installed successfully in service worker');
} else {
    console.log('🔍 Buffer already available in service worker, skipping polyfill');
}
...
(() => {
  ...
  // src/background.js
  console.log("\u{1F7E2} Zap Social Background script with REAL nostr-tools loaded");
  console.log("\u{1F527} nostr-tools functions available:", {
    generateSecretKey: typeof generateSecretKey,
    getPublicKey: typeof getPublicKey,
    finalizeEvent: typeof finalizeEvent,
    nip19: typeof nip19_exports
  });
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    console.log("\u{1F4E8} Background received message:", request);
  ...
  async function handleNostrRequest(request, sendResponse) {
    ...
    console.log("\u{1F527} Handling NOSTR request:", method);
    try {
      switch (method) {
        case "getPublicKey":
  ...
  async function handleGetPublicKey(sendResponse) {
    ...
    chrome.storage.local.get(["nostr_public_key", "logged_out"], (result) => {
      // Check if user is logged out
      if (result.logged_out) {
        console.log("🚪 User is logged out, not providing public key");
        sendResponse({ error: "User is logged out. Please log in through the extension popup." });
        return;
      }
  ...
  async function handleSignEvent(event, sendResponse) {
    ...
    chrome.storage.local.get(["nostr_private_key", "nostr_public_key", "logged_out"], async (result) => {
      // Check if user is logged out
      if (result.logged_out) {
        console.log("🚪 User is logged out, not signing event");
        sendResponse({ error: "User is logged out. Please log in through the extension popup." });
        return;
      }
      ...
      try {
        console.log("\u{1F511} Using stored keys:", {
        ...
        const privateKeyBytes = hexToBytes2(result.nostr_private_key);
        console.log("\u{1F510} Calling REAL finalizeEvent...");
  ...
  async function handleDerivePublicKey(privateKeyHex, sendResponse) {
    try {
      console.log("\u{1F511} Deriving public key from private key with REAL nostr-tools...");
      const privateKeyBytes = hexToBytes2(privateKeyHex);
      console.log("\u2705 Private key converted to bytes:", privateKeyBytes.length, "bytes");
      console.log("\u{1F527} Calling REAL getPublicKey...");
      const publicKey = getPublicKey(privateKeyBytes);
      console.log("\u2705 Public key derived:", publicKey.length, "chars");
      console.log("\u{1F527} Calling REAL nip19.npubEncode...");
      const npub = nip19_exports.npubEncode(publicKey);
      console.log("\u2705 npub generated:", npub.substring(0, 20) + "...");
      console.log("\u{1F527} Calling REAL nip19.nsecEncode...");
      const nsec = nip19_exports.nsecEncode(privateKeyBytes);
  ...
  async function handleImportPrivateKey(privateKeyInput, sendResponse) {
    try {
      console.log("\u{1F4E5} Importing private key with REAL nostr-tools...", {
      ...
      if (privateKeyInput.startsWith("nsec")) {
        console.log("\u{1F527} Decoding nsec with REAL nip19.decode...");
        const decoded = nip19_exports.decode(privateKeyInput);
        if (decoded.type !== "nsec") {
          throw new Error("Invalid nsec format");
        }
        privateKeyBytes = decoded.data;
        privateKeyHex = bytesToHex2(privateKeyBytes);
        ...
        privateKeyBytes = hexToBytes2(privateKeyHex);
      ...
      // Clear all existing data first, then set new identity
  ...
  async function handleGenerateKeys(sendResponse) {
    try {
      console.log("\u{1F511} Generating new keypair with REAL nostr-tools...");
      console.log("\u{1F527} Calling REAL generateSecretKey...");
  ...
  async function handleLogout(sendResponse) {
    try {
      console.log("🚪 Logging out user - clearing identity but preserving keys...");
      
      // Set logout state while preserving keys in storage
      chrome.storage.local.set({
        logged_out: true,
        logged_out_at: Date.now()
      }, () => {
        console.log("✅ User logged out successfully");
        sendResponse({ result: "Logged out successfully" });
      });
    } catch (error) {
  ...
  chrome.runtime.onInstalled.addListener(() => {
    console.log("\u{1F7E2} Zap Social NOSTR extension installed with REAL nostr-tools");
    console.log("\u{1F9EA} Testing REAL nostr-tools functions...");
    try {
  ...
  console.log("\u{1F7E2} Background script loaded with REAL nostr-tools");
})();
